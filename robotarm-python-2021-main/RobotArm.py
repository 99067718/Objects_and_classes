...
########### ANIMATION METHODS ###########
  ...
  def getStackSize(self):
    ...
    return len(self._yard[self._stack])

  def moveStackTo(self, position: int):
    currentStack = self._stack
    currentStackSize = len(self._yard[self._stack])
    if position-currentStack <0:
      movesLeft = currentStack - position -1
      self.destroyCurrentBlock()
      for i in range(currentStackSize):
        self.grab()
        self.moveLeftTimes(movesLeft)
        self.drop()
        self.moveRightTimes(movesLeft) ##########################################################################################################
    else:
      movesLeft = position - currentStack -1
      self.destroyCurrentBlock()
      for i in range(currentStackSize):
        self.grab()
        self.moveRightTimes(movesLeft)
  ...
  def getNextColorRight(self, color : str):
    currentPos = self._stack
    possibleMoves = 10 - self._stack
    for i in range(possibleMoves):
      self.grab()
      currentColor = self.scan()
